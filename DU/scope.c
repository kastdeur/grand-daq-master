...
uint8_t shadowlist[Reg_End];  //!< all parameters to set in FPGA
uint8_t shadowlistR[Reg_End]; //!< all parameters read from FPGA
...
/*!
 \func void scope_raw_write(unsigned char *buf, int len)
 \brief writes a buffer to the digitizer
 \param buf pointer to the data to send
 \param           len number of bytes to send
 */
void scope_raw_write(uint16_t *buf,int nwords) //new, writing to AXI... buf[0] = AXI-register buf[1]=register-address buf[2]=data
{
  int32_t itot=0,i;
  
#ifndef Fake
  if((write(dev,&buf[2],1)< 1) {
#ifdef DEBUG_SCOPE
    ...
    break;
      ...
      */
     int scope_raw_read(uint16_t *bf, int nwords) //new, reading from AXI
     {
    int ir;
#ifndef Fake
    ir =read(dev, (void *)bf, 1);
#else
    ir = 0;
    usleep(100);
#endif
    ...
    return(1);
      ...
      */
     void scope_close() //is this needed?
     {
#ifndef Fake
    if(close(dev)<0){
#ifdef DEBUG_SCOPE
     ...
     {
    uint16_t buf[3];
    buf[0] =Reg_Dig_Control>>2;
    buf[1] = Reg_Dig_Control;
    buf[2] = (*(uint16_t *)&shadowlist[Reg_Dig_Control]) |(CTRL_PPS_EN | CTRL_SEND_EN );
    scope_set_parameters(buf,1);
    seczero = 0;
  }
...
#ifdef Fake
#endif
    ptr = (uint16_t *)&shadowlist[data[1]];
    if(to_shadow == 1)*ptr=data[2];
    scope_raw_write(data,1);
    usleep(1000);
  }
    ...
    for(int i=0;i<Reg_End;i++){
      buf[0] = i>>2;
      buf[1] = i;
      buf[2] = *(uint16_t *)&shadowlist[i];
      scope_set_parameters(buf,0);
    }
    ...
    for(int ich=0;ich<4;ich++){
      if(shadowlist[Reg_TestPulse_ChRead]&(1<<ich)){
    ...
    memcpy(&evtbuf[offset+EVT_CTRL],&shadowlist[Reg_Dig_Control],8*sizeof(uint16_t));
    memcpy(&evtbuf[offset+EVT_WINDOWS],&shadowlist[Reg_Time1_Pre],8*sizeof(uint16_t));
    memcpy(&evtbuf[offset+EVT_CHANNEL],&shadowlist[Reg_ADC1_Gain],4*6*sizeof(uint16_t));
    memcpy(&evtbuf[offset+EVT_TRIGGER],&shadowlist[Reg_Trig1_ThresA],4*6*sizeof(uint16_t));
    memcpy(&evtbuf[offset+EVT_FILTER1],&shadowlist[Reg_FltA1_A1],4*4*8*sizeof(uint16_t));
    //followed by ADC....
    if(Send_10 ==0){
      Send_10 = 1;
    }else{
    ...
    *(shm_ev.next_write) = next_write;
    ...
    uint16_t statusbuffer[4]={Reg_GenStatus>>2,Reg_GenStatus,0,0};
    uint16_t ctrlbuffer[4]={Reg_GenControl>>2,Reg_GenControl,0,0};
