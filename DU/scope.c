 ...
 */
void scope_write(uint16_t *buf) //new, writing to AXI... buf[0] = AXI-register buf[1]=register-address buf[2]=data
{
  int32_t itot=0,i;
 ...
 */
int scope_raw_read(uint16_t *bf) //new, reading from AXI
{
  int ir;
#ifndef Fake
...
void scope_set_parameters(uint16_t *data, int to_shadow)
{
  ...
  for(i=0;i<3;++i) printf(" %04x",data[i]);
  printf("\n");
