...
unsigned short iirNotchPipe2(int reset, unsigned short in, double nu, double r) {
    ...
    // 14x12 signed multiply... truncate output to 24b????
...
unsigned short iirNotchPipe4(int reset, unsigned short in, double nu, double r) {
    ...
    if (((fw3_1 >> 24) != 0) && ((fw3_1 >> 24) != -1))
        fprintf(stderr, "WARNING! fw3_1 OVERFLOW (0x%0x)\n", fw3_1);

    if (((fw3_2 >> 24) != 0) && ((fw3_2 >> 24) != -1))
    ...
    if (((fw2_out >> 13) & 0x1) != ((fw2_out >> 14) & 0x1))
...
//
// Note: this filter seems to have a much wider notch for a given
// r.  This could be a bug in my implementation.
//
// Retains state from one call to next to implement feedforward 
// and feedback.
//
